...
#pragma warning restore 0414
        ...
        public static void RemoveListener(string rMessageType, MessageHandler rHandler, bool rImmediate, bool forceRemove = false)
        {
            RemoveListener(rMessageType, "", rHandler, rImmediate, forceRemove);
        }
        ...
        /// <param name="rHandler">Hander to handle the message</param>
        /// <param name="rImmediate">Determines if the function ignores the cache and forcibly removes the listener from the list now</param>
        public static void RemoveListener(string rMessageType, string rFilter, MessageHandler rHandler, bool rImmediate,bool forceRemove=false)
        {
            ...
            if (forceRemove)
            {
                if (mMessageHandlers.ContainsKey(lListener.MessageType))
                {
                   mMessageHandlers.Remove(lListener.MessageType);
                }
                MessageListenerDefinition.Release(lListener);
            }
            else if (rImmediate)
