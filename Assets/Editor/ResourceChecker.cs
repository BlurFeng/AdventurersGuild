...
public class ResourceChecker : EditorWindow 
{
	...
	void OnGUI ()
	{
		...
		GUI.color = defColor;
		GUILayout.BeginArea(new Rect(position.width - 110, 10, 100, 75));
		if (GUILayout.Button("Start Check",GUILayout.Width(100), GUILayout.Height(50)))
			CheckResources();
		if (GUILayout.Button("CleanUp",GUILayout.Width(100), GUILayout.Height(20)))
		...
		if (thingsMissing)
		{
			GUILayout.Label("Missing-" + MissingObjects.Count);
		}
	...
	void CheckResources()
	{
		...
		if (IncludeScriptReferences)
		{
			...
			foreach (MonoBehaviour script in scripts)
			{
				...
				//获取所有引用Missing的物体
				List<Object> missingObjs = new List<Object>();
                var sObj = new SerializedObject(script);
                ...
                while (iter.NextVisible(true))//如果有下一个属性
                {
                    ...
                    if (iter.propertyType == SerializedPropertyType.ObjectReference)
                    {
                        ...
                        {
							if (!missingObjs.Contains(script.gameObject))
                            {
								missingObjs.Add(script.gameObject);
								MissingGraphic tMissing = new MissingGraphic();
                                tMissing.Object = script.transform;
                                tMissing.type = iter.displayName;
                                tMissing.name = script.gameObject.name;
                                MissingObjects.Add(tMissing);
                                thingsMissing = true;
                            }
                            break;
